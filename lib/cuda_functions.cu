...
__global__ void StaticWaveGeneration_single (double* __restrict__ frequency, double* pnOut,short** sumOut)
    {
        /// in this code we introduce a linear ramp 
        size_t i = blockDim.x * blockIdx.x + threadIdx.x;
        for (int ch=0;ch<channel_num_cuda;ch++){
            float sum = 0.;
            for (size_t j=0;j<static_num_cuda[ch];j++){
                float ramp_time = static_cast<float>(i) / illSamplerate_cuda; // Temps en secondes
                float ramp_factor = ramp_time / 25000; // Facteur de la rampe entre 0 et 1
                float ramped_amplitude = amplitude_signal * ramp_factor; // Amplitude avec la rampe
                int index = tone_count_cuda[ch]+j;
                float phi = __fmul_rn(__fmul_rn(2.,istatic_num_cuda[ch]),static_cast<float>( j*j));
                float ampl = __fmul_rn(__fmul_rn(ramped_amplitude, __sinf(__fmaf_rn(__fmul_rn(__fmul_rn(2.0, M_PI), frequency[index]), ramp_time, phi))), istatic_num_cuda[ch]);
