...
#include <iostream>
#include <thread>
#define USING_EXTERNAL_TRIGGER  1
using namespace std;
...
void cuda_cleanup()
{
    ...
    {
        if (summed_buffer[i] != NULL)
            cudaFree(summed_buffer[i]);
        if (final_buffer[i] != NULL)
            cudaFree(final_buffer[i]);
        if (saved_buffer[i] != NULL)
            cudaFree(saved_buffer[i]);
        if (dynamic_saved_buffer[i] != NULL)
            cudaFree(dynamic_saved_buffer[i]);
    }
    if (destination_buffer_cuda != NULL)
        cudaFree(destination_buffer_cuda);
    if (summed_buffer_cuda != NULL)
        cudaFree(summed_buffer_cuda);
    if (final_buffer_cuda != NULL)
        cudaFree(final_buffer_cuda);
    if (saved_buffer_cuda != NULL)
        cudaFree(saved_buffer_cuda);
    if (dynamic_saved_buffer_cuda != NULL)
        cudaFree(dynamic_saved_buffer_cuda);
    if (dynamic_list_cuda != NULL)
        cudaFree(dynamic_list_cuda);
    if (static_buffer_cuda != NULL)
...
int main()
{
    ...
    StaticWaveGeneration<<<(static_length / lThreadsPerBlock), lThreadsPerBlock>>>(real_static_freq_cuda, static_buffer_cuda, summed_buffer_cuda);
    cudaDeviceSynchronize();
    ...
    if (dynamic_total)
    {
        ...
        cudaDeviceSynchronize();
        if (eCudaErr = cudaPeekAtLastError())
            printf("3CUDA Error Peek!!: %s\n", cudaGetErrorString(eCudaErr));
    ...
    if (dynamic_total)
    {
        ...
        for (int cnt = 0; cnt < dynamic_loopcount && !iskeypressed; cnt++)
        {
            ...
            else
            {
                ...
                AccelCombine<<<static_length / lThreadsPerBlock / 2, lThreadsPerBlock>>>(sPointerPosition, saved_buffer_cuda, summed_buffer_cuda, dFreq_cuda, dDiff_cuda, dphi_cuda, ratio_cuda);
                cudaDeviceSynchronize();
